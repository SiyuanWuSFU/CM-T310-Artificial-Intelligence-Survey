    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for Ghost_position in self.legalPositions:
            position_dis = self.getPositionDistribution(self.setGhostPosition(gameState, Ghost_position))
            for position, prob in position_dis.items():
                newBelief[position] += prob * self.beliefs[Ghost_position]
        self.beliefs.clear()               
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = []                                                                                                                                                                                                                                                                                      
        even = int(self.numParticles / len(self.legalPositions))
    ...
    def observe(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        allPossible = util.Counter()
        new_belief = self.getBeliefDistribution()
        flag = False
        temp = []
        i = 0
        if noisyDistance == None:
            ...
            for p in self.legalPositions:
                ...
                allPossible[p] += emissionModel[Distance]*new_belief[p]

            if  sum(allPossible.values())==0:
                self.initializeUniformly(gameState)
            else: 
                while i in range (0,self.numParticles):
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particles:
            dist = self.getPositionDistribution(self.setGhostPosition(gameState, particle))           
            temp.append(util.sample(dist))
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for ghost in range(self.numGhosts):
                dist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), ghost, self.ghostAgents[ghost])
                          
                newParticle[ghost] = util.sample(dist)
