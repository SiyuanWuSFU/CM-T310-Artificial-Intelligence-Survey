        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                if count != 4:
                    ...
                    #print(track)
                tuple1 = ((nextx,nexty),nc)
                sustuple = (tuple1,action,1)
                
                successors.append(sustuple)
...
def cornersHeuristic(state, problem):
    ...
    for c in corners: 
        ...
        corner = 0
        for i in visited:
            distance.append( util.manhattanDistance(current_N,corners[corner]))
            corner += 1
        ...
        for distance_close in distance:
            if visited[closest] == 1:
                closest = corner
            
            if  visited[corner] == 0 and distance_close < distance[closest]:
                closest = corner
            corner += 1

        if  visited[closest] ==0:
            heuristic += distance[closest]
            current_N = corners[closest]
            visited[closest]=1
